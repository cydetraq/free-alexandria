...
def main() -> int:
    ...
    parser.add_argument(
        "--strict-links",
        action="store_true",
        help="fail while linked-only recommendations still use generic search URLs",
    )
    ...
    if cross_category_dupes:
        errors.append(
            "canonical-lock.json: IDs appear in multiple editorial categories: "
            + ", ".join(cross_category_dupes)
        )
    ...
    if args.strict_links:
        try:
            text = CURATED_READING_PATH.read_text(encoding="utf-8")
        except FileNotFoundError:
            errors.append("docs/curated-reading.md is missing")
        else:
            generic_hits = sorted(
                pattern for pattern in GENERIC_SEARCH_PATTERNS if pattern in text.lower()
            )
            if generic_hits:
                errors.append(
                    "linked-only list still contains generic primary search URLs: "
                    + ", ".join(generic_hits)
                )
            ...
            missing_linked_ids = sorted(set(lock.get("linked_only", [])) - documented_ids)
            if missing_linked_ids:
                errors.append(
                    "linked-only canonical IDs are not explicitly documented in curated reading data: "
                    + ", ".join(missing_linked_ids)
                )
