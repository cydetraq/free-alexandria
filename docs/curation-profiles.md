...
Profiles do not contain files, URLs that must work at runtime, or ad-hoc copies of catalog metadata. They reference local catalog record IDs and state their language, rights, and storage policy.
...
- `distribution` fails unless each selected work resolves through `catalog/published-editions.json` to a local edition with matching hashes, provenance, a dated operator evidence note, and a size within the profile limit. It then copies those verified files into the release.
...
Profiles can choose among reviewed catalog records. They cannot override rights status, turn an external link into a mirror, select a restricted government publication, or omit required provenance.
